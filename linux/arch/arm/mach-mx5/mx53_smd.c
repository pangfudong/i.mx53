...
#define MX53_SMD_KEY_BACK			(0*32 + 2)	/* GPIO1_2 */
...
#define MX53_SMD_KEY_VOLUMEDOWN		(1*32 + 14)	/* GPIO2_14 */
#define MX53_SMD_KEY_VOLUMEUP		(1*32 + 15)	/* GPIO2_15 */
...
#define MX53_SMD_LCD_RL			(1*32 + 30)	/* GPIO_2_30 */
...
#define MX53_SMD_WL_REG_ON			(2*32 + 4)	/*GPIO_3_4 */
...
#define MX53_SMD_LCD_UD			(2*32 + 17)	/*GPIO_3_17 */
#define MX53_SMD_CAP_TCH_INT1		(2*32 + 20)	/* GPIO_3_20 */
#define MX53_SMD_WL_RESET_N			(2*32 + 21)	/* GPIO_3_21 */
...
#define MX53_SMD_WL_WLAN_WAKE		(6*32 + 13)	/* GPIO_7_13 */
...
#ifdef CONFIG_TOUCHSCREEN_RAYDIUM
static int ts_get_irq_level(void)
{
	return gpio_get_value(MX53_SMD_CAP_TCH_INT0);
}

static struct ts_platform_data raydium_ts_data = {
	.mode = TS_MODE_INT_LOW,
	.irq = gpio_to_irq(MX53_SMD_CAP_TCH_INT0),
...
{
	gpio_request(MX53_SMD_CHA_ISET, "cam-pwr");
	gpio_direction_output(MX53_SMD_CHA_ISET, pwdn);
	gpio_free(MX53_SMD_CHA_ISET);
}
...
static void ite661x_hdmi_reset(void)
{
...
static struct mxc_lcd_platform_data ite661x_hdmi_data = {
	.reset = ite661x_hdmi_reset,
	.boot_enable = 1,
};
...
static int mxc_rt5621_amp_enable(int enable)
{
	return 0;
}
...
{
	return gpio_get_value(MX53_SMD_HEADPHONE_DEC);
}

static int mxc_rt5621_init(void)
{
	gpio_direction_input(MX53_SMD_HEADPHONE_DEC);
	return 0;
}

static struct mxc_audio_platform_data rt5621_data = {
	.ssi_num = 1,
	.src_port = 2,
	.ext_port = 5,
	.hp_irq = gpio_to_irq(MX53_SMD_HEADPHONE_DEC),
	.hp_status = headphone_det_status,
	.amp_enable = mxc_rt5621_amp_enable,
	.init = mxc_rt5621_init,
};

static struct platform_device mxc_rt5621_device = {
	.name = "imx-3stack-rt5621",
};
...
static void __init mx53_smd_io_init(void)
{
	...
	gpio_request(MX53_SMD_PMIC_INT, "pmic-irq");
	gpio_direction_input(MX53_SMD_PMIC_INT);

	gpio_request(MX53_SMD_MODEM_WKUP, "lcd-power+");
	gpio_direction_output(MX53_SMD_MODEM_WKUP, 1);

	gpio_request(MX53_SMD_MODEM_RESET_B, "lcd-power-");
	gpio_direction_output(MX53_SMD_MODEM_RESET_B, 1);

	gpio_request(MX53_SMD_USB_HUB_RESET_B, "lcd-bl-en");
	gpio_direction_output(MX53_SMD_USB_HUB_RESET_B, 1);

	gpio_request(MX53_SMD_ZIGBEE_RESET_B, "tp-en");
	gpio_direction_output(MX53_SMD_ZIGBEE_RESET_B, 0);

	gpio_request(MX53_SMD_LCD_PWR_EN, "lcd-pwr-en");
	gpio_direction_output(MX53_SMD_LCD_PWR_EN, 1);

	/* USB OTG */
	gpio_request(MX53_SMD_WAKEUP_ZIGBEE, "usb-pwr-ctl");
	gpio_direction_output(MX53_SMD_WAKEUP_ZIGBEE, 0);
	gpio_request(MX53_SMD_MODEM_DISABLE_B, "vcc-en");
	gpio_direction_output(MX53_SMD_MODEM_DISABLE_B, 0);

	/* CHG-STAT */
	gpio_request(MX53_SMD_CAP_TCH_INT1, "chg-stat");
	gpio_direction_input(MX53_SMD_CAP_TCH_INT1);

	/* DC-DET */
	gpio_request(MX53_SMD_DC_DET, "dc-det");
	gpio_direction_input(MX53_SMD_DC_DET);

	/* LCD up and down */
	gpio_request(MX53_SMD_LCD_UD, "lcd-ud");
	gpio_direction_output(MX53_SMD_LCD_UD, 0);

	/* LCD right and left */
	gpio_request(MX53_SMD_LCD_RL, "lcd-rl");
	gpio_direction_output(MX53_SMD_LCD_RL, 0);
	...
	/* ITE661x HDMI controller */
	gpio_request(MX53_SMD_HDMI_PWRON, "disp0-pwr-en");
	gpio_direction_output(MX53_SMD_HDMI_PWRON, 0);
	...
	/* Touchscreen reset */
	gpio_request(MX53_SMD_CAP_TCH_FUN0, "touch-reset");
	gpio_direction_output(MX53_SMD_CAP_TCH_FUN0, 0);
	msleep(100);
	gpio_direction_output(MX53_SMD_CAP_TCH_FUN0, 1);
	...
	gpio_request(MX53_SMD_WL_WLAN_WAKE, "wl-wlan-wake");
	gpio_direction_output(MX53_SMD_WL_WLAN_WAKE, 1);
