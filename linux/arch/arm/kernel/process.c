...
#define FSG_NAMELEN 24
#define onyx_attr(_name) \
...
int    wfm_mode = 3;
EXPORT_SYMBOL(screen_width);
EXPORT_SYMBOL(screen_height);
EXPORT_SYMBOL(wfm_mode);

static int __init video_setup(char *options)
{
    int grayscale;
    if (sscanf(options, "%d*%d:%d", &screen_width, &screen_height, &grayscale) != 3)
    {
        return -EINVAL;
    }
...
static void bs60_ld_value(int v)
{
	u8* tmp = kmalloc(screen_width * screen_height, GFP_KERNEL);
	if (tmp == NULL)
	{
		return;
	}

	memset(tmp, v & 0xff, screen_width * screen_height);
	bs_cmd_ld_img_area_data_with_stride(3, 0, 0, screen_width, screen_height, tmp, screen_width);
	kfree(tmp);
}

static void bs60_white(void)
{
	bs60_ld_value(0xFF);
	bs_cmd_wait_for_bit(0x338, 0, 0);
	bs_cmd_wait_for_bit(0x338, 3, 0);
	bs_cmd_upd_full(wfm_mode, 0, 0);
	bs_cmd_wait_for_bit(0x338, 0, 0);
	bs_cmd_wait_for_bit(0x338, 3, 0);
}
...
static ssize_t clear_at_poweroff_store(struct kobject *kobj,
	struct kobj_attribute *attr, const char *buf, size_t n)
{
	int val;

	if (sscanf(buf, "%d", &val) == 1) {
		clear_at_poweroff = !!val;
		return n;
	}
	return -EINVAL;
}

#define WDT_BASE                0x53FDC000
...
#define heartbeat_timer_interval 200

static struct timer_list onyx_heartbeat_timer;

static void heartbeat_func(unsigned long param)
{
	u32 wdt_base_reg = IO_ADDRESS(WDT_BASE);

	__raw_writew(WDT_MAGIC_1, wdt_base_reg + MXC_WDT_WSR);
	__raw_writew(WDT_MAGIC_2, wdt_base_reg + MXC_WDT_WSR);

	printk("Sent heartbeat signal to watchdog...\n");
	mod_timer(&onyx_heartbeat_timer, jiffies + heartbeat_timer_interval);
}
...
static void start_watchdog(void)
{
	...
	init_timer(&onyx_heartbeat_timer);
	onyx_heartbeat_timer.expires = jiffies + heartbeat_timer_interval;
    onyx_heartbeat_timer.function = heartbeat_func;
    add_timer(&onyx_heartbeat_timer);

	clk_enable(clk_get(NULL, "wdog_clk"));

	/* Configure watchdog, set timeout to 3sec */
	__raw_writew(0x0873, wdt_base_reg + MXC_WDT_WCR);
	__raw_writew(0x0877, wdt_base_reg + MXC_WDT_WCR);
}

static void stop_watchdog(void)
{
	clk_disable(clk_get(NULL, "wdog_clk"));
	del_timer(&onyx_heartbeat_timer);
}
...
static int __init onyx_set_sys_entry(void)
{
	struct kobject* onyx_kobj = kobject_create_and_add("onyx", NULL);
	...
	return sysfs_create_group(onyx_kobj, &attr_group);
}

late_initcall(onyx_set_sys_entry);
