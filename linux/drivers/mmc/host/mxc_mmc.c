...
extern void mxc_wifi_hardreset(struct work_struct* work);
extern int mmc_schedule_delayed_work(struct delayed_work *work,
				     unsigned long delay);
static struct delayed_work wifi_reset_work;
...
static irqreturn_t mxcmci_gpio_irq(int irq, void *devid)
{
	...
	if (host->id == 1 && card_gpio_status == 1)
	{
		mmc_schedule_delayed_work(&wifi_reset_work, 0);
	}

	host->plat_data->card_inserted_state = card_gpio_status ? 0 : 1;
	if (card_gpio_status == 1 && host->req)
	{
		/* The SD card is removed, but we have a pending request. */
...
static int __init mxcmci_init(void)
{
	...
	INIT_DELAYED_WORK(&wifi_reset_work, mxc_wifi_hardreset);
