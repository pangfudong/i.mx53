...
BYTE VideoFormat ;
// BYTE bVideoInputType = 0 /* | T_MODE_CCIR656 | T_MODE_SYNCEMB | T_MODE_INDDR */ ; // for Sync Embedded, CCIR656, InputDDR


// ULONG AudioSampleFreq = 48000 ;
// BYTE AudioSWL = 24 ;
// BYTE AudioChannel = 2 ;
BYTE bOutputAudioMode = 0x41 ; // use full packet as default
BYTE bAudioChannelSwap = 0 ;
BYTE bIntType = 0 ;
BYTE bAudioEnableSetting = 0 ;
BYTE bAudioPendingCounter = 0 ;
ULONG currVideoPixelClock ;
...
static BOOL bPendingAudioAdjust = FALSE ;
 ...
 unsigned short ManualCheckRiCounter = 0 ;
...
// Function Prototype
////////////////////////////////////////////////////////////////////////////////
void InitCAT6611(struct i2c_client *client);
...
BYTE HDMITX_ReadI2C_Byte(SHORT RegAddr)
{
	...
	ret = i2c_smbus_read_byte_data(g_client, RegAddr);
	return ret < 0 ? ER_FAIL : ret;
}
...
SYS_STATUS HDMITX_WriteI2C_Byte(SHORT RegAddr, BYTE Data)
{
	...
	ret = i2c_smbus_write_byte_data(g_client, RegAddr, Data);
	return ret < 0 ? ER_FAIL : ER_SUCCESS;
}
...
SYS_STATUS HDMITX_ReadI2C_ByteN(SHORT RegAddr, BYTE *pData, int N)
{
	...
	ret = i2c_smbus_read_i2c_block_data(g_client, RegAddr, N, pData);
	return ret < 0 ? ER_FAIL : ER_SUCCESS;
}
...
SYS_STATUS HDMITX_WriteI2C_ByteN(SHORT RegAddr, BYTE *pData, int N)
{
	...
	ret = i2c_smbus_write_i2c_block_data(g_client, RegAddr, N, pData);
	return ret < 0 ? ER_FAIL : ER_SUCCESS;
}
...
void
InitCAT6611(struct i2c_client *client)
{
    printk(KERN_ERR "InitCAT6611():\n") ;
...
ProgramSyncEmbeddedVideoMode(BYTE VIC, BYTE bInputSignalType)
{
    ...
    printk(KERN_ERR "ProgramSyncEmbeddedVideoMode(%d , %d)\n",VIC, bInputSignalType) ;
    ...
    //    HDMITX_SetReg_Byte(REG_HVPol,~0x6,(SyncEmbTable[i].RegHVPol)&6) ; // Reg90
        ...
        if(bInputSignalType & T_MODE_CCIR656) {
    		printk(KERN_ERR "Embedded,  CCIR8\n");
    	...
    	{
            printk(KERN_ERR "Embedded,  CCIR16\n");
    ...
    {
        BYTE *pTimingGen ;
        printk(KERN_ERR "Generate Sync and DE by Sync\n") ;
        ...
        case 22:
            ...
            break ;
        default:
            printk(KERN_ERR "Not a supported timing for VIC %d to gen sync/DE from sync.\n",VIC) ;
            return FALSE ;
        }
...
BOOL ProgramDEGenModeByID(MODE_ID id,BYTE bInputSignalType)
{
    ...
    if( (bInputSignalType & (T_MODE_DEGEN|T_MODE_SYNCGEN|T_MODE_SYNCEMB) )==(T_MODE_DEGEN))
    {
        ...
        Switch_HDMITX_Bank(0) ;	//printk(KERN_ERR "\n===========\n");
        HDMITX_WriteI2C_Byte(0x90,DeGen_Table[i].Reg90) ;// printk(KERN_ERR "Reg0x90=%X \n",DeGen_Table[i].Reg90);
        HDMITX_WriteI2C_Byte(0x92,DeGen_Table[i].Reg92) ;// printk(KERN_ERR "Reg0x92=%X \n",DeGen_Table[i].Reg92);
        HDMITX_WriteI2C_Byte(0x93,DeGen_Table[i].Reg93) ;// printk(KERN_ERR "Reg0x93=%X \n",DeGen_Table[i].Reg93);
        HDMITX_WriteI2C_Byte(0x94,DeGen_Table[i].Reg94) ;// printk(KERN_ERR "Reg0x94=%X \n",DeGen_Table[i].Reg94);
        HDMITX_WriteI2C_Byte(0x9A,DeGen_Table[i].Reg9A) ;// printk(KERN_ERR "Reg0x9A=%X \n",DeGen_Table[i].Reg9A);
        HDMITX_WriteI2C_Byte(0x9B,DeGen_Table[i].Reg9B) ;// printk(KERN_ERR "Reg0x9B=%X \n",DeGen_Table[i].Reg9B);
        HDMITX_WriteI2C_Byte(0x9C,DeGen_Table[i].Reg9C) ;// printk(KERN_ERR "Reg0x9C=%X \n",DeGen_Table[i].Reg9C);
        HDMITX_WriteI2C_Byte(0x9D,DeGen_Table[i].Reg9D) ;// printk(KERN_ERR "Reg0x9D=%X \n",DeGen_Table[i].Reg9D);
        HDMITX_WriteI2C_Byte(0x9E,DeGen_Table[i].Reg9E) ;// printk(KERN_ERR "Reg0x9E=%X \n",DeGen_Table[i].Reg9E);
        HDMITX_WriteI2C_Byte(0x9F,DeGen_Table[i].Reg9F) ;// printk(KERN_ERR "Reg0x9F=%X \n",DeGen_Table[i].Reg9F);
        return TRUE ;
...
EnableVideoOutput(BOOL HiFreq, unsigned char inputColorMode, unsigned char inputSignalType, unsigned char outputColorMode, BOOL bHDMI)
{
    ...
    printk(KERN_ERR "EnableVideoOutput()\n") ;
...
//     if(bHDMI)
//     {
//         printk(KERN_ERR "InitVideoMode(): SetHDMIMode\n") ;
//         HDMITX_WriteI2C_Byte(REG_HDMI_MODE,B_HDMI_MODE) ;
//     }
//     else
//     {
//         printk(KERN_ERR "InitVideoMode(): SetDVIMode\n") ;
//         HDMITX_WriteI2C_Byte(REG_HDMI_MODE,B_DVI_MODE) ;
//     }
    SetHDMIMode(bHDMI) ;
    ...
    if (i == 100)
    	printk(KERN_ERR "Wait B_TXVIDSTABLE timed out!\n");
...
CheckHDMITX(BYTE *pHPD, BYTE *pHPDChange)
{
    ...
    if(sysstat & B_INT_ACTIVE)
    {
        ...
        printk(KERN_ERR "INT_Handler: reg%02x = %02x\n",REG_INT_STAT1,intdata1) ;

		if( intdata1 & B_INT_DDCFIFO_ERR )
		{
		    printk(KERN_ERR "DDC FIFO Error.\n") ;
		    ClearDDCFIFO() ;
		}
		...
		{
            printk(KERN_ERR "interrupt Authenticate Done.\n") ;
		...
		{
            printk(KERN_ERR "interrupt Authenticate Fail.\n") ;
			AbortDDC();   // @emily add
            HDCP_ResumeAuthentication() ;
        }
        #endif // SUPPORT_HDCP
...
SetInputMode(BYTE InputColorMode, BYTE InputSignalType)
{
    ...
    printk(KERN_ERR "SetInputColorMode(%02X, %02X)\n",InputColorMode,InputSignalType) ;
    ...
    {
        ucData |= B_2X656CLK ; printk(KERN_ERR "CCIR656 mode\n") ;
    }

    if( InputSignalType & T_MODE_SYNCEMB )
    {
        ucData |= B_SYNCEMB ; printk(KERN_ERR "Sync Embedded mode\n") ;
    }

    if( InputSignalType & T_MODE_INDDR )
    {
        ucData |= B_INDDR ; printk(KERN_ERR "Input DDR mode\n") ;
    }
...
SetCSCScale(BYTE bInputMode,BYTE bOutputMode)
{
    ...
    case F_MODE_YUV444:
        printk(KERN_ERR "Input mode is YUV444 ") ;
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            printk(KERN_ERR "Output mode is YUV444\n") ;
            csc = B_CSC_BYPASS ;
            break ;

        case F_MODE_YUV422:
            printk(KERN_ERR "Output mode is YUV422\n") ;
            ...
            break ;
        case F_MODE_RGB444:
            printk(KERN_ERR "Output mode is RGB444\n") ;
            csc = B_CSC_YUV2RGB ;
    ...
    case F_MODE_YUV422:
        printk(KERN_ERR "Input mode is YUV422\n") ;
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            printk(KERN_ERR "Output mode is YUV444\n") ;
    ...
    #ifdef SUPPORT_INPUTRGB
    case F_MODE_RGB444:
        printk(KERN_ERR "Input mode is RGB444\n") ;
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            printk(KERN_ERR "Output mode is YUV444\n") ;
    ...
    {
        printk(KERN_ERR "CSC = YUV2RGB %x ",csc) ;
...
//    BYTE uc ;
    ...
    printk(KERN_ERR "reg[%02X] = %02X",REG_AFE_DRV_CTRL,HDMITX_ReadI2C_Byte(REG_AFE_DRV_CTRL)) ;
    printk(KERN_ERR "reg[%02X] = %02X ",REG_AFE_XP_CTRL1,HDMITX_ReadI2C_Byte(REG_AFE_XP_CTRL1)) ;
    printk(KERN_ERR "reg[%02X] = %02X ",REG_AFE_XP_CTRL2,HDMITX_ReadI2C_Byte(REG_AFE_XP_CTRL2)) ;
    printk(KERN_ERR "reg[%02X] = %02X ",REG_AFE_IP_CTRL,HDMITX_ReadI2C_Byte(REG_AFE_IP_CTRL)) ;
...
SetAudioFormat(BYTE NumChannel, BYTE AudioEnable, ULONG SampleFreq, BYTE AudSWL, BYTE AudioCatCode)
{
    ...
    printk(KERN_ERR "SetAudioFormat(%d channel,%02X,SampleFreq %ldHz,AudSWL %d,%02X)\n", NumChannel,  AudioEnable,  SampleFreq,  AudSWL,  AudioCatCode) ;
    ...
    // printk(KERN_ERR "Sampling Freq = %ld fs = %x\n",SampleFreq,fs) ;
...
SetNCTS(ULONG PCLK, ULONG Fs)
{
    ...
    printk(KERN_ERR "SetNCTS(%ld,%ld): MCLK = %ld\n",PCLK,Fs,MCLK) ;
...
ReadEDID(BYTE *pData, BYTE bSegment, BYTE offset, SHORT Count)
{
    ...
    // printk(KERN_ERR "ReadEDID(%08lX, %d, %d, %d)\n",(ULONG)pData, bSegment, offset, Count) ;
    if( !pData )
    {
        printk(KERN_ERR "ReadEDID(): Invallid pData pointer %08lX\n",(ULONG)pData) ;
        return ER_FAIL ;
    }

    if(HDMITX_ReadI2C_Byte(REG_INT_STAT1) & B_INT_DDC_BUS_HANG)
    {
    	printk(KERN_ERR "Called AboutDDC()\n") ;
        AbortDDC() ;
    ...
    while( RemainedCount > 0 )
    {
        ...
        printk(KERN_ERR "ReadEDID(): ReqCount = %d, bCurrOffset = %d\n",ReqCount, bCurrOffset ) ;
        ...
        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut -- )
        {
            ...
            {
                printk(KERN_ERR "ReadEDID(): DDC_STATUS = %02X, fail.\n",ucdata) ;
                return ER_FAIL ;
            }
        }
        ...
        if( TimeOut == 0 )
        {
            printk(KERN_ERR "ReadEDID(): DDC TimeOut. \n") ;
            return ER_FAIL ;
        }
...
SetAVIInfoFrame(AVI_InfoFrame *pAVIInfoFrame)
{
	...
	printk(KERN_ERR "SetAVIInfo(): ") ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB1)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB2)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB3)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB4)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB5)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB6)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB7)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB8)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB9)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB10)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB11)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB12)) ;
    //printk(KERN_ERR "%02X ",HDMITX_ReadI2C_Byte(REG_AVIINFO_DB13)) ;
	printk(KERN_ERR "\n") ;
...
HDCP_Auth_Fire(void)
{
    // printk(KERN_ERR "HDCP_Auth_Fire():\n") ;
...
HDCP_GetBCaps(PBYTE pBCaps ,PUSHORT pBStatus)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_DONE)
        {
            //printk(KERN_ERR "HDCP_GetBCaps(): DDC Done.\n") ;
            break ;
        }

        if(ucdata & B_DDC_ERROR)
        {
            printk(KERN_ERR "HDCP_GetBCaps(): DDC fail by reg16=%02X.\n",ucdata) ;
            return ER_FAIL ;
        }
    }
...
HDCP_GetBKSV(BYTE *pBKSV)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_ERROR)
        {
            printk(KERN_ERR "HDCP_GetBCaps(): DDC No ack or arbilose,%x,maybe cable did not connected. Fail.\n",ucdata) ;
            return ER_FAIL ;
        }
    }
...
HDCP_Authenticate(void)
{
    ...
    printk(KERN_ERR "HDCP_Authenticate():\n") ;
	HDCP_Reset() ;
    // ClearDDCFIFO() ;
    // AbortDDC() ;
    ...
    for( TimeOut = 50 ; TimeOut > 0 ; TimeOut-- )
    {
        ...
        {
            printk(KERN_ERR "HDCP_GetBCaps fail.\n") ;
            return ER_FAIL ;
        }
    	...
    	if(bHDMIMode)
    	{
    		...
    		{
    			printk(KERN_ERR "Sink HDCP in DVI mode over HDMI,do not authenticate and encryption. BCaps = %x BStatus = %x\n",BCaps,BStatus) ;
    		}
    	}
    	else
    	{
    		...
    		{
    			printk(KERN_ERR "Sink HDCP in HDMI mode over DVI,do not authenticate and encryption. BCaps = %x BStatus = %x\n",BCaps,BStatus) ;
    		}
    	}
    	msleep(5) ;
    }
    ...
    if(TimeOut == 0 )
    {
    	if(bHDMIMode)
    	{
    		if((BStatus & B_CAP_HDMI_MODE)==0)
    		{
    			printk(KERN_ERR "Not a HDMI mode,do not authenticate and encryption. BCaps = %x BStatus = %x\n",BCaps,BStatus) ;
    			return ER_FAIL ;
    		}
    	}
    	else
	...
	printk(KERN_ERR "BCaps = %02X BStatus = %04X\n",BCaps,BStatus) ;
    /*
    if((BStatus & M_DOWNSTREAM_COUNT)> 6)
    {
        printk(KERN_ERR "Down Stream Count %d is over maximum supported number 6,fail.\n",(BStatus & M_DOWNSTREAM_COUNT)) ;
        return ER_FAIL ;
    }
    */

    HDCP_GetBKSV(BKSV) ;
	printk(KERN_ERR "BKSV %02X %02X %02X %02X %02X\n",BKSV[0],BKSV[1],BKSV[2],BKSV[3],BKSV[4]) ;
    ...
    if(revoked)
    {
        printk(KERN_ERR "BKSV is revoked\n") ; return ER_FAIL ;
    }
    printk(KERN_ERR "BKSV %02X %02X %02X %02X %02X is NOT %srevoked\n",BKSV[0],BKSV[1],BKSV[2],BKSV[3],BKSV[4],revoked?"not ":"") ;
    ...
    //{
    //    printk(KERN_ERR "RX support HDCP 1.1\n") ;
    //    HDMITX_WriteI2C_Byte(REG_HDCP_DESIRE,B_ENABLE_HDPC11|B_CPDESIRE) ;
    //}
    //else
    //{
    //    printk(KERN_ERR "RX not support HDCP 1.1\n") ;
    ...
    if((BCaps & B_CAP_HDMI_REPEATER) == 0)
    {
        ...
        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut --)
        {
            ...
            printk(KERN_ERR "reg46 = %02x reg16 = %02x\n",ucdata,HDMITX_ReadI2C_Byte(0x16)) ;
            ...
            if(ucdata & B_INT_AUTH_FAIL)
            {
                ...
                */
                printk(KERN_ERR "HDCP_Authenticate(): Authenticate fail\n") ;
                bAuthenticated = FALSE ;
                return ER_FAIL ;
            }
        }
        ...
        if(TimeOut == 0)
        {
             printk(KERN_ERR "HDCP_Authenticate(): Time out. return fail\n") ;
             bAuthenticated = FALSE ;
             return ER_FAIL ;
        }
        return ER_SUCCESS ;
...
void SHATransform(ULONG * h)
{
	...
	for (t = 0; t < 20; t++) {
		...
		// printk(KERN_ERR "%08lX %08lX %08lX %08lX %08lX\n",h[0],h[1],h[2],h[3],h[4]) ;
...
void SHA_Simple(void *p,LONG len,BYTE *output)
{
    ...
    for( i = 0 ; i < len ; i++ )
    {
        ...
        c <<= (3-(i%4))*8 ;
        w[t] |= c ;
        // printk(KERN_ERR "pBuff[%d] = %02x, c = %08lX, w[%d] = %08lX\n",i,pBuff[i],c,t,w[t]) ;
    }
    t = i/4 ;
    if( i%4 == 0 )
    {
...
{
    printk(KERN_ERR "HDCP_CancelRepeaterAuthenticate") ;
    HDMITX_WriteI2C_Byte(REG_DDC_MASTER_CTRL,B_MASTERDDC|B_MASTERHOST) ;
    AbortDDC() ;
...
HDCP_GetKSVList(BYTE *pKSVList,BYTE cDownStream)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_DONE)
        {
            printk(KERN_ERR "HDCP_GetKSVList(): DDC Done.\n") ;
            break ;
        }

        if(ucdata & B_DDC_ERROR)
        {
            printk(KERN_ERR "HDCP_GetKSVList(): DDC Fail by REG_DDC_STATUS = %x.\n",ucdata) ;
            return ER_FAIL ;
        }
        msleep(5) ;
    }
...
HDCP_GetVr(BYTE *pVr)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_DDC_DONE)
        {
            printk(KERN_ERR "HDCP_GetVr(): DDC Done.\n") ;
            break ;
        }

        if(ucdata & B_DDC_ERROR)
        {
            printk(KERN_ERR "HDCP_GetVr(): DDC fail by REG_DDC_STATUS = %x.\n",ucdata) ;
            return ER_FAIL ;
        }
        msleep(5) ;
    }
    ...
    if(TimeOut == 0)
    {
        printk(KERN_ERR "HDCP_GetVr(): DDC fail by timeout.\n") ;
        return ER_FAIL ;
    }
...
HDCP_Authenticate_Repeater(void)
{
	...
	printk(KERN_ERR "Authentication for repeater\n") ;
    //////////////////////////////////////
    // Authenticate Fired
    //////////////////////////////////////
    ...
    for(TimeOut = 250*6 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(uc & B_INT_AUTH_FAIL)
        {
            ...
            HDMITX_WriteI2C_Byte(REG_SYS_STATUS,0) ;
			*/
            printk(KERN_ERR "HDCP_Authenticate_Repeater(): B_INT_AUTH_FAIL.\n") ;
            goto HDCP_Repeater_Fail ;
        }
        // emily add for test
        ...
        if(uc & B_INT_KSVLIST_CHK)
        {
            ...
            HDMITX_WriteI2C_Byte(REG_SYS_STATUS,B_INTACTDONE) ;
            HDMITX_WriteI2C_Byte(REG_SYS_STATUS,0) ;
            printk(KERN_ERR "B_INT_KSVLIST_CHK\n") ;
            break ;
        }
    ...
    if(TimeOut == 0)
    {
        printk(KERN_ERR "Time out for wait KSV List checking interrupt\n") ;
        goto HDCP_Repeater_Fail ;
    }
    ...
    for(TimeOut = 500 ; TimeOut > 0 ; TimeOut --)
    {
		if((TimeOut % 100) == 0)
		{
		    printk(KERN_ERR "Wait KSV FIFO Ready %d\n",TimeOut) ;
		}

        if(HDCP_GetBCaps(&BCaps,&BStatus) == ER_FAIL)
        {
            printk(KERN_ERR "Get BCaps fail\n") ;
            goto HDCP_Repeater_Fail ;
        }

        if(BCaps & B_CAP_KSV_FIFO_RDY)
        {
			 printk(KERN_ERR "FIFO Ready\n") ;
			 break ;
        }
        msleep(5) ;
    ...
    if(TimeOut == 0)
    {
        printk(KERN_ERR "Get KSV FIFO ready TimeOut\n") ;
        goto HDCP_Repeater_Fail ;
    }

	printk(KERN_ERR "Wait timeout = %d\n",TimeOut) ;
    ...
    {
        printk(KERN_ERR "Invalid Down stream count,fail\n") ;
        goto HDCP_Repeater_Fail ;
    }
...
//        if( ucdata & B_DDC_ERROR )
//        {
//            printk(KERN_ERR "HDCP_GetBCaps(): DDC Error. Reg16 = %02X.\n", ucdata) ;
//            return ER_FAIL ;
//        }
//
//    }
