...
#define assert(expr) ((void) ((expr) ? 0 : (printk("assert failed at line %d",__LINE__))))
...
#define MSLEEP(x) hal_delay_us(1000 * x)
#endif
...
static void reserve_gpio_pins(void)
{
    mxc_request_iomux(MX31_PIN_GPIO3_0, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_GPIO3_1, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_SCLK0, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_SRST0, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D4, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D5, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D6, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D7, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D8, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D9, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D10, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D11, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D12, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D13, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D14, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_D15, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_MCLK, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_VSYNC, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_HSYNC, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_CSI_PIXCLK, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_SD_D_I, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_SD_D_IO, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_SD_D_CLK, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_LCS0, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_LCS1, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_SER_RS, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_ATA_CS0, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_ATA_CS1, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_ATA_DIOR, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_ATA_DIOW, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_ATA_DMACK, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_request_iomux(MX31_PIN_ATA_RESET_B, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    
    mxc_iomux_set_pad(MX31_PIN_ATA_RESET_B, PAD_CTL_100K_PD | PAD_CTL_PUE_PUD);
}
...
#else
    unsigned int x = __raw_readl(GPIO3_BASE_ADDR + 0x04);
    __raw_writel((x & ~(1 << pin)) | (val << pin), GPIO3_BASE_ADDR + 0x04);
#endif
}
...
void set_gpio_val( int pin, int val )
{
    ...
    *GPIO_DR = ((*GPIO_DR) & ~(1 << pin)) | (val << pin);
    ...
    __raw_writel((x & ~(1 << pin)) | (val << pin), GPIO3_BASE_ADDR);
#endif
}
...
#else
        unsigned int x = __raw_readl(GPIO3_BASE_ADDR + 0x04);
        __raw_writel(x | 0x0000ffff, GPIO3_BASE_ADDR + 0x04);
#endif
        hdb_dir = 1;
    }
    else
...
#else
        unsigned int x = __raw_readl(GPIO3_BASE_ADDR + 0x04);
        __raw_writel(x & 0xffff0000, GPIO3_BASE_ADDR + 0x04);
#endif
        hdb_dir = 0;
    }
}
...
#else
    return __raw_readl(GPIO3_BASE_ADDR) & 0xffff;
#endif
}
...
void wr_data( int value )
{
    ...
    *GPIO_DR |= VAL_CS_WE_1;
#else
    unsigned int x = __raw_readl(GPIO3_BASE_ADDR) & VAL_HDR;
    int count = 0;
    while ( x == 0 && count++ < WAIT_MAX)
    {
        x = __raw_readl(GPIO3_BASE_ADDR) & VAL_HDR;
    }
    ...
    // data
    x = __raw_readl(GPIO3_BASE_ADDR);
    __raw_writel((x & 0xffff0000) | (value & 0x0000ffff), GPIO3_BASE_ADDR);
...
void wr_gpio_hdb( int val )
{
    ...
    *GPIO_DR = (*GPIO_DR & 0xffff0000) | (val & 0x0000ffff);
    ...
    {
        unsigned int x = __raw_readl(GPIO3_BASE_ADDR);
        __raw_writel((x & 0xffff0000) | (val & 0x0000ffff), GPIO3_BASE_ADDR);
    }
#endif
}
