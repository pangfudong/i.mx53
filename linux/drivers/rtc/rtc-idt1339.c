...
*/
#	define IDT1339_REG_A1SECS	0x07
#	define IDT1339_REG_A1MIN	0x08
#	define IDT1339_REG_A1HOUR	0x09
#	define IDT1339_REG_A1DATE	0x0a
...
static int idt1339_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		case RTC_AIE_ON:
		case RTC_AIE_OFF:
			msg->addr = idt1339->client->addr;
			msg->flags = 0;
			msg->len = 3;
			msg->buf = data;
			data[0] = IDT1339_REG_CONTROL;
			data[1] = (cmd == RTC_AIE_ON) ? 0x05 : 0x04;
			data[2] = 0;
			result = i2c_transfer(idt1339->client->adapter, msg, 1);
			if (result != 1)
			{
				dev_err(dev, "i2c_transfer: write to IDT1339_REG_CONTROL failed\n");
				ret = -EIO;
			}
			break;
		default:
...
static int idt1339_get_time(struct device *dev, struct rtc_time *t)
{
	...
	tmp = i2c_transfer(idt1339->client->adapter, idt1339->msg, 2);
	if (tmp != 2) {
...
static int idt1339_set_time(struct device *dev, struct rtc_time *t)
{
	...
	*buf++ = 0;		/* first register addr */
	...
	result = i2c_transfer(idt1339->client->adapter,	&idt1339->msg[1], 1);
	if (result != 1) {
		dev_err(dev, "%s error %d\n", "write", tmp);
		return -EIO;
	}
...
static int idt1339_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	struct idt1339* idt1339 = dev_get_drvdata(dev);
	struct i2c_msg msg[1];
	u8 data[5];
	...
	msg->len = 5;
	msg->buf = data;
	data[0] = IDT1339_REG_A1SECS;
	data[1] = bin2bcd(alrm->time.tm_sec);
	data[2] = bin2bcd(alrm->time.tm_min);
	data[3] = bin2bcd(alrm->time.tm_hour);
	data[4] = bin2bcd(alrm->time.tm_mday);

	result = i2c_transfer(idt1339->client->adapter, msg, 1);
	if (result != 1)
	{
		dev_err(dev, "i2c_transfer: write to IDT1339_REG_A1SECS failed\n");
		return -EIO;
	}
...
static ssize_t
idt1339_nvram_read(struct file *filp, struct kobject *kobj, struct bin_attribute *attr,
...
static ssize_t
idt1339_nvram_write(struct file *flip, struct kobject *kobj, struct bin_attribute *attr,
		char *buf, loff_t off, size_t count)
{
	struct i2c_client	*client;
