...
#define WM831X_LDO_MODE_NORMAL    0
#define WM831X_LDO_MODE_LOW_POWER 1
...
static int wm831x_stby_enable(struct regulator_dev *rdev)
{
	struct wm831x_ldo *ldo = rdev_get_drvdata(rdev);
	struct wm831x *wm831x = ldo->wm831x;
	int vsel = 0x0;
	...
	return wm831x_set_bits(wm831x, reg,
					WM831X_LDO1_SLP_SLOT_MASK,
					vsel << WM831X_LDO1_SLP_SLOT_SHIFT);
}

static int wm831x_stby_disable(struct regulator_dev *rdev)
{
	struct wm831x_ldo *ldo = rdev_get_drvdata(rdev);
	struct wm831x *wm831x = ldo->wm831x;
	int vsel = 0x1;
...
static int wm831x_stby_set_mode(struct regulator_dev *rdev, unsigned int mode)
{
	struct wm831x_ldo *ldo = rdev_get_drvdata(rdev);
	struct wm831x *wm831x = ldo->wm831x;
	int vsel;
	...
	switch (mode) {
	case REGULATOR_MODE_NORMAL:
		vsel = WM831X_LDO_MODE_NORMAL;
		break;
	case REGULATOR_MODE_IDLE:
		vsel = WM831X_LDO_MODE_LOW_POWER;
		break;
	default:
		return -EINVAL;
	}
	...
	return wm831x_set_bits(wm831x, reg,
					WM831X_LDO1_SLP_MODE_MASK,
					vsel << WM831X_LDO1_SLP_MODE_SHIFT);
}
