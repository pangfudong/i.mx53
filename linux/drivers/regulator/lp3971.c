...
extern void mxc_init_dptc(void);
...
static int lp3971_ldo_is_enabled(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int ldo = dev->id - LP3971_LDO1;
	u16 mask = 1 << (1 + ldo);
	u16 val;
...
static int lp3971_ldo_enable(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int ldo = dev->id - LP3971_LDO1;
	u16 mask = 1 << (1 + ldo);
...
static int lp3971_ldo_disable(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int ldo = dev->id - LP3971_LDO1;
	u16 mask = 1 << (1 + ldo);
...
static int lp3971_ldo_get_voltage(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int ldo = dev->id - LP3971_LDO1;
	u16 val, reg;
...
static int lp3971_ldo_set_voltage(struct regulator *dev, int uV)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int ldo = dev->id - LP3971_LDO1;
	int mV = uV / 1000;
	...
	if (mV < vol_map[LDO_VOL_MIN_IDX] || mV > vol_map[LDO_VOL_MAX_IDX])
		return -EINVAL;
...
static int lp3971_dcdc_is_enabled(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int buck = dev->id - LP3971_DCDC1;
	u16 mask = 1 << (buck * 2);
	u16 val;
...
static int lp3971_dcdc_enable(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int buck = dev->id - LP3971_DCDC1;
	u16 mask = 1 << (buck * 2);
...
static int lp3971_dcdc_disable(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int buck = dev->id - LP3971_DCDC1;
	u16 mask = 1 << (buck * 2);
...
static int lp3971_dcdc_get_voltage(struct regulator *dev)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int buck = dev->id - LP3971_DCDC1;
	u16 reg;
	...
	if (reg <= BUCK_TARGET_VOL_MAX_IDX)
		...
		val = 0;
		dev_warn(&dev->cdev, "chip reported incorrect voltage value.\n");
	}

	return val;
}

static int lp3971_dcdc_set_voltage(struct regulator *dev, int uV)
{
	struct lp3971 *lp3971 = regulator_get_drvdata(dev);
	int buck = dev->id - LP3971_DCDC1;
	int mV = uV / 1000;
	...
	if (mV < vol_map[BUCK_TARGET_VOL_MIN_IDX] ||
	    mV > vol_map[BUCK_TARGET_VOL_MAX_IDX])
		return -EINVAL;
...
extern void set_reference_voltage(int ref);
			    ...
			    const struct i2c_device_id *id)
{
	...
	{
		ret = regulator_register(&regulators[i]);
		if (ret < 0)
		{
			dev_err(lp3971->dev, "Failed to register regulator %s\n",
				regulators[i].name);
			goto unwind_reg;
		}
