...
static int set_mic1_to_hifiad_path(struct rt5621_route_control *ctrl, int enable)
{
	struct snd_soc_codec *codec = ctrl->private_data;
	int  micvolume = 0x1A;
	//printk("set_mic1_to_hifiad_path\n");
	if (enable)
	{
		ctrl->state = 1;
...
static int set_hifida_to_hp_path(struct rt5621_route_control *ctrl, int enable)
{
	...
	if (enable)
	{
		...
		rt5621_write_mask(codec, RT5621_STEREO_DAC_VOL, 0x0000, 0x8000);					/*dac to hp mixer*/
		rt5621_write_mask(codec, RT5621_OUTPUT_MIXER_CTRL, 0x0300, 0x0300);				/*hp mux*/
		...
		hp_depop_mode2(codec);
		rt5621_write_mask(codec, RT5621_HP_OUT_VOL, 0x0000, 0x8080);                                      /*unmute hp*/
	}
	else
	{
	...
	struct snd_soc_dai *codec_dai)
{
	...
	coeff = get_coeff(rt5621->sysclk, params_rate(params));
	//printk("sysclk: %d, params_rate(params):%d", rt5621->sysclk, params_rate(params));
...
static int rt5621_init(struct snd_soc_device *socdev)
{
	...
	rt5621_write(codec, RT5621_JD_CTRL_REG, 0x4180);//enable Vref
