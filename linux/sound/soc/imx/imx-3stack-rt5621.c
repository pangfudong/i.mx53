/*
 * imx-3stack-rt5621.c  --  i.MX 3Stack Driver for Freescale RT5621 Codec
 *
...
static unsigned int rt5621_rates[] = {
	0,
	32000,
	44100,
	48000,
...
/*
 * imx_3stack RT5621 audio DAI opserations.
 */
...
static void imx_3stack_init_dam(int ssi_port, int dai_port)
{
	...
	/* RT5621 uses SSI1 or SSI2 via AUDMUX port dai_port for audio */
...
static int rt5621_jack_func;
static int rt5621_spk_func;
static int rt5621_line_in_func;
...
static int rt5621_get_jack(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = rt5621_jack_func;
	return 0;
}

static int rt5621_set_jack(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (rt5621_jack_func == ucontrol->value.enumerated.item[0])
		return 0;

	rt5621_jack_func = ucontrol->value.enumerated.item[0];
	if (rt5621_jack_func)
...
static int rt5621_get_spk(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = rt5621_spk_func;
	return 0;
}

static int rt5621_set_spk(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (rt5621_spk_func == ucontrol->value.enumerated.item[0])
		return 0;

	rt5621_spk_func = ucontrol->value.enumerated.item[0];
	if (rt5621_spk_func)
...
static int rt5621_get_line_in(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = rt5621_line_in_func;
	return 0;
}

static int rt5621_set_line_in(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);

	if (rt5621_line_in_func == ucontrol->value.enumerated.item[0])
		return 0;

	rt5621_line_in_func = ucontrol->value.enumerated.item[0];
	if (rt5621_line_in_func)
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	asrc_ssi_data.output_sample_rate = rt5621_rates[asrc_func];
...
static int imx_3stack_rt5621_init(struct snd_soc_codec *codec)
{
	int i, ret;
	...
	for (i = 0; i < ARRAY_SIZE(rt5621_machine_controls); i++) {
		ret = snd_ctl_add(codec->card,
				  snd_soc_cnew(&rt5621_machine_controls[i],
					       codec, NULL));
		if (ret < 0)
			return ret;
	}
...
static struct rt5621_setup_data rt5621_setup_data = {
	.i2c_bus = 1,
	.i2c_address = 0x1A,
}; 
...
static int imx_3stack_rt5621_remove(struct platform_device *pdev)
{
	struct mxc_audio_platform_data *plat = pdev->dev.platform_data;
	struct imx_3stack_priv *priv = &card_priv;
...
static struct platform_driver imx_3stack_rt5621_audio_driver = {
	.probe = imx_3stack_rt5621_probe,
	.remove = imx_3stack_rt5621_remove,
	.driver = {
...
static int __init imx_3stack_init(void)
{
	...
	ret = platform_driver_register(&imx_3stack_rt5621_audio_driver);
	if (ret)
		return -ENOMEM;
...
{
	platform_driver_unregister(&imx_3stack_rt5621_audio_driver);
...
MODULE_DESCRIPTION("RT5621 Driver for i.MX 3STACK");
MODULE_LICENSE("GPL");
