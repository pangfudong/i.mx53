/*
 * imx-3stack-wm8731.c  --  i.MX 3Stack Driver for Wolfson WM8731 Codec
 *
				     ...
				     struct snd_pcm_hw_params *params)
{
	...
	codec_dai->ops->set_sysclk(codec_dai, WM8731_SYSCLK, 11289600, SND_SOC_CLOCK_IN);
...
/*
 * imx_3stack WM8731 HiFi DAI opserations.
 */
...
{
	/* WM8731 uses SSI1 or SSI2 via AUDMUX port dai_port for audio */
...
static int imx_3stack_pcm_new(struct snd_soc_pcm_link *pcm_link)
{
	...
	printk(KERN_INFO "i.MX 3STACK WM8731 Audio Driver\n");
...
#ifdef CONFIG_PM
static int imx_3stack_wm8731_audio_suspend(struct platform_device *dev,
					   pm_message_t state)
{
	return 0;
}

static int imx_3stack_wm8731_audio_resume(struct platform_device *dev)
{
	return 0;
}

#else
#define imx_3stack_wm8731_audio_suspend	NULL
...
static int __devinit imx_3stack_wm8731_audio_probe(struct platform_device *pdev)
{
	...
	if (tmp->src_port == 2)
		hifi = snd_soc_pcm_link_new(machine, "imx_3stack-hifi",
					    ...
					    wm8731_codec_name, wm8731_dai_name,
					    imx_ssi_3);
	else
		hifi = snd_soc_pcm_link_new(machine, "imx_3stack-hifi",
					    &imx_3stack_pcm_ops, imx_pcm,
...
imx_3stack_wm8731_audio_remove(struct platform_device *pdev)
{
	struct snd_soc_machine *machine = platform_get_drvdata(pdev);
	struct snd_soc_codec *codec;
	struct snd_soc_pcm_link *pcm_link;
...
static struct platform_driver imx_3stack_wm8731_audio_driver = {
	.probe = imx_3stack_wm8731_audio_probe,
	.remove = __devexit_p(imx_3stack_wm8731_audio_remove),
	.suspend = imx_3stack_wm8731_audio_suspend,
	.resume = imx_3stack_wm8731_audio_resume,
	.driver = {
		   .name = imx_3stack_audio,
		   },
};

static int __init imx_3stack_wm8731_audio_init(void)
{
	return platform_driver_register(&imx_3stack_wm8731_audio_driver);
}

static void __exit imx_3stack_wm8731_audio_exit(void)
{
	platform_driver_unregister(&imx_3stack_wm8731_audio_driver);
}

module_init(imx_3stack_wm8731_audio_init);
module_exit(imx_3stack_wm8731_audio_exit);

MODULE_AUTHOR("Liam Girdwood");
MODULE_DESCRIPTION("WM8731 Machine Driver for i.MX 3STACK");
MODULE_LICENSE("GPL");
