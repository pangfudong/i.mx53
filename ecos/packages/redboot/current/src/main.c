...
#define BLK_MEM_ADDR        0x100000
#define BLK_SIZE            0x20000
#define BLK_SIZE_WITH_SPARE 0x21000
#define NON_ROOTFS_BLOCKS   56
#define SYSCONFIG_BUF       0x121000
#define SYSCONFIG_LEN       2048
#define SYSCONFIG_ADDR      0x1FF800
#define REDBOOT_TMP         0x200000
#define REDBOOT_BLKS        16

#define SYSCONFIG_COMPLETE      0x1258
#define ROOTFS_COMPLETE         0x1259
#define WAVEFORM_CMD_COMPLETE   0x1260
#define WAVEFORM_DATA_COMPLETE  0x1261

static void read_sysconfig(struct onyx_sysconfig* cfg)
{
    void *err_addr;
    flash_read((void *)SYSCONFIG_ADDR, (void *)cfg, sizeof(struct onyx_sysconfig), (void **)&err_addr);

    /* Sanity check */
    if (cfg->screen_size != 60 && cfg->screen_size != 80 && cfg->screen_size != 97)
    {
        cfg->screen_size = 60;
        cfg->x_pixels = 600;
        cfg->y_pixels = 800;
...
{
    static cyg_uint32 blk_p = BLK_MEM_ADDR;
    static cyg_uint32 blk_index = 0;
    static cyg_uint32 wfm_cmd_len = 0;

    u32 buffer_end = BLK_MEM_ADDR + (blk_index < NON_ROOTFS_BLOCKS ? BLK_SIZE : BLK_SIZE_WITH_SPARE);

    if (len == 8)
    {
        cyg_uint32 code = *(cyg_uint32 *)buf;
        cyg_uint32 is_msg = 1;
        switch (code)
        {
            case SYSCONFIG_COMPLETE:
                memcpy((cyg_uint8 *)SYSCONFIG_BUF, (cyg_uint8 *)BLK_MEM_ADDR, SYSCONFIG_LEN);
                blk_p = BLK_MEM_ADDR;
                diag_printf("End programming sysconfig...\n");
                break;

            case ROOTFS_COMPLETE:
                /* The rootfs transfer is complete */
                if (blk_p != BLK_MEM_ADDR)
                {
                    onyx_program_blk(blk_index++, BLK_MEM_ADDR, blk_p - BLK_MEM_ADDR, 1);
                    blk_p = BLK_MEM_ADDR;
                }
                diag_printf("End programming rootfs...\n");
                break;

            case WAVEFORM_CMD_COMPLETE:
                program_wfm_flash(0, blk_p - BLK_MEM_ADDR, (cyg_uint8 *)BLK_MEM_ADDR);
                wfm_cmd_len = blk_p - BLK_MEM_ADDR;
                blk_p = BLK_MEM_ADDR;
                diag_printf("End programming waveform cmd...\n");
                break;

            case WAVEFORM_DATA_COMPLETE:
                program_wfm_flash(wfm_cmd_len, blk_p - BLK_MEM_ADDR, (cyg_uint8 *)BLK_MEM_ADDR);
                blk_p = BLK_MEM_ADDR;
                diag_printf("End programming waveform data...\n");
    ...
    if (blk_p + len < buffer_end)
    {
        memcpy((cyg_uint8 *)blk_p, buf, len);
        blk_p += len;
    }
    else
    {
        cyg_uint32 pad = buffer_end - blk_p;
        memcpy((cyg_uint8 *)blk_p, buf, pad);

        if (blk_index < REDBOOT_BLKS)
        {
            memcpy((u8 *)(REDBOOT_TMP + BLK_SIZE * blk_index),
                (u8 *)BLK_MEM_ADDR,
                BLK_SIZE);

            if (blk_index == REDBOOT_BLKS - 1)
            {
                int i;
                memcpy((u8 *)(REDBOOT_TMP + REDBOOT_BLKS * BLK_SIZE - SYSCONFIG_LEN),
                    (u8 *)SYSCONFIG_BUF,
                    SYSCONFIG_LEN);

                rebuild_bbt();

                for (i = 0; i < REDBOOT_BLKS; i++)
                {
                    onyx_program_blk(i, REDBOOT_TMP + i * BLK_SIZE, BLK_SIZE, 0);
                }
            }
        }
        else
        {
            // We have a new erase block now
            onyx_program_blk(blk_index,
                BLK_MEM_ADDR,
                buffer_end - BLK_MEM_ADDR,
                blk_index < NON_ROOTFS_BLOCKS ? 0 : 1);
        }

        blk_index++;

        memcpy((cyg_uint8 *)BLK_MEM_ADDR, buf + pad, len - pad);
        blk_p = BLK_MEM_ADDR + len - pad;
    }
}
...
static void eink_init(struct onyx_sysconfig* cfg, int want_update)
{
    int i = 0;
    void *err_addr;

    switch (cfg->screen_size)
    {
        case 60:
            bs60_init( 0x886 );
            break;
    ...
    if (want_update > 0)
    {
        ...
        int start_x = 2;
        int start_y = cfg->y_pixels - 2 - UPDIMG_HEIGHT;

        for (i=0; i<UPDIMG_HEIGHT; i++)
        {
            memcpy(tmp + cfg->x_pixels * (start_y + i) + start_x, upd_image[i], UPDIMG_WIDTH);
        }
    }
...
cyg_start(void)
{
    ...
    // Update pmic if necessary.
    int retry = 0;
    for(; retry < 3; ++retry)
    {
        if (enable_3971_ldo5(1) == 0)
        {
            break;
        }
    }
    ...
    mxc_request_iomux(MX31_PIN_KEY_ROW4, OUTPUTCONFIG_GPIO, INPUTCONFIG_GPIO);
    mxc_set_gpio_direction(MX31_PIN_KEY_ROW4, 1);
    if (mxc_get_gpio_datain(MX31_PIN_KEY_ROW4))
    {
        res = 5000;
    }
    ...
    if (want_program)
    {
        usbs_imx_otg_device_init(i);
        register_rx_callback(usb_rx);
        usbs_imx_otg_download();
    }
